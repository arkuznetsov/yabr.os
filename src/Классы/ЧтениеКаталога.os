...
Перем СортироватьПоИмени;        // Булево            - Истина - найденные файлы будут отсортированы по имени
                                 ...
                                 //                     (устаревший, используется параметр "ПорядокСортировкиПоИмени")
Перем ПорядокСортировкиПоИмени;  // Строка            - asc - файлы сортируются по возрастанию имени
...
Функция ОписаниеПараметров() Экспорт
	...
	ДобавитьОписаниеПараметра(Параметры,
	                          ...
	                          ,
	                          Истина,
	                          "(устаревший, используйте параметр ""ПорядокСортировкиПоИмени"")
	                          |Истина - найденные файлы будут отсортированы по имени
	                          |         отдельно для каждой указанной маски файлов.");
	ДобавитьОписаниеПараметра(Параметры,
	                          "ПорядокСортировкиПоИмени",
	                          "Строка",
	                          ,
	                          "no",
	                          "asc - найденные файлы будут отсортированы по возрастанию имени
	                          |desc - найденные файлы будут отсортированы по убыванию имени
	                          |<прочее> (по умолчанию) - сортировка не выполняется
	                          |       сортировка выполняется в пределах каждой указанной маски файлов.");
...
Процедура УстановитьПараметрыОбработкиДанных(Знач НовыеПараметры) Экспорт
	...
	Если ПараметрыОбработки.Свойство("ПорядокСортировкиПоИмени") Тогда
		ПорядокСортировкиПоИмени = ПараметрыОбработки.ПорядокСортировкиПоИмени;
	Иначе
		ПорядокСортировкиПоИмени = "no";
	КонецЕсли;
...
Процедура УстановитьПараметрОбработкиДанных(Знач ИмяПараметра, Знач Значение) Экспорт
	...
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("ПорядокСортировкиПоИмени") Тогда
		ПорядокСортировкиПоИмени = Значение;
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьДанные() Экспорт
	...
	Для Каждого ТекМаска Из МассивМасок Цикл
		...
		Если ВРег(ПорядокСортировкиПоИмени) = "ASC" Тогда
			СписокФайловПоМаске.СортироватьПоПредставлению(НаправлениеСортировки.Возр);
		ИначеЕсли ВРег(ПорядокСортировкиПоИмени) = "DESC" Тогда
			СписокФайловПоМаске.СортироватьПоПредставлению(НаправлениеСортировки.Убыв);
		КонецЕсли;
