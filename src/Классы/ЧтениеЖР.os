...
Перем ВыгружатьКолонки;        // Структура              - Содержит список колонок с указанием необходимости их выгрузки
		// *__ПоУмолчанию         Булево                 - Истина - если для колонок не указан флаг выгрузки,
		//                                                 то они будут выгружены
		//                                               - Ложь - если для колонок не указан флаг выгрузки,
		//                                                 то они не будут выгружены
...
Процедура ОбработатьДанные() Экспорт
	...
	Если НакопленныеДанные.Количество() >= РазмерПорцииОбработки Тогда
		...
		Если ТипЗнч(НакопленныеДанные) = Тип("Массив") Тогда
			НакопленныеДанные.Очистить();
		КонецЕсли;
...
Функция РазобратьЭлемент(Элемент)
	...
	ВремяТранзакцииВМиллисекундах = Строка16ВЧисло10(Элемент.Значения[2].Значения[0]);
	ДобавитьЗначениеКолонкиВЗапись(Запись,
								   "ВремяТранзакцииВМиллисекундах",
								   ВремяТранзакцииВМиллисекундах);
	ДобавитьЗначениеКолонкиВЗапись(Запись,
								   "ВремяТранзакции",
								   Дата(1, 1, 1, 0, 0, 0) + ВремяТранзакцииВМиллисекундах / 10000);
	...
	Если ВремДанные.Количество() > 1 Тогда
		Если ТипЗнч(ВремДанные[1]) = Тип("Структура") И ТипДанных = "P" Тогда
			Для й = 1 По ВремДанные[1].Значения.ВГраница() Цикл
				ОписаниеДанных = ВремДанные[1].Значения[й].Значения;
				Если ОписаниеДанных.Количество() > 1 Тогда
					ДанныеДляЗаписи.Добавить(ОбработатьКавычкиВСтроке(ОписаниеДанных[1]));
				КонецЕсли;
			КонецЦикла;
		Иначе
			Если ВремДанные.Количество() > 1 Тогда
	...
	ОсновнойПорт = ПолучитьЗначениеИзСловаря("ОсновныеПорты", Число(Элемент.Значения[14]));
	Если ЗначениеЗаполнено(ОсновнойПорт) Тогда
		ОсновнойПорт = Число(ОсновнойПорт);
	Иначе
		ОсновнойПорт = 0;
	КонецЕсли;
	ДобавитьЗначениеКолонкиВЗапись(Запись, "ОсновнойПорт", ОсновнойПорт);

	ВспомогательныйПорт = ПолучитьЗначениеИзСловаря("ВспомогательныеПорты", Число(Элемент.Значения[14]));
	Если ЗначениеЗаполнено(ВспомогательныйПорт) Тогда
		ВспомогательныйПорт = Число(ВспомогательныйПорт);
	Иначе
		ВспомогательныйПорт = 0;
	КонецЕсли;
	ДобавитьЗначениеКолонкиВЗапись(Запись, "ВспомогательныйПорт", ВспомогательныйПорт);

	ДобавитьЗначениеКолонкиВЗапись(Запись, "Сеанс", Число(Элемент.Значения[16]));
...
//
// Возвращаемое значение:
//	Булево         - Истина - элемент является записью журнала;
//	                 Ложь - в противном случае
//
