...
Перем Эластик_Соединение;      // HTTP-соединение        - объект соединения с http-сервисом
...
Процедура УстановитьПараметрыОбработкиДанных(Знач НовыеПараметры) Экспорт
	...
	УстановитьАдресСервераЭластик();

	Эластик_Путь = "";
	Если ПараметрыОбработки.Свойство("Эластик_Путь") И ЗначениеЗаполнено(ПараметрыОбработки.Эластик_Путь) Тогда
...
Процедура УстановитьПараметрОбработкиДанных(Знач ИмяПараметра, Знач Значение) Экспорт
	...
	Если Найти(ВРег("Эластик_Сервер, Эластик_Порт"), ВРег(ИмяПараметра)) > 0 Тогда
		УстановитьАдресСервераЭластик();
	ИначеЕсли ВРег(ИмяПараметра) = ВРег("Эластик_Путь") И ЗначениеЗаполнено(Значение) Тогда
...
Процедура ЗавершениеОбработкиДанных() Экспорт
	...
	Если БылиОшибкиОтправки Тогда
		Лог.Ошибка("[%1]: При отправке данных (%2) в ""%3:%4%5"" возникли ошибки!",
		            ТипЗнч(ЭтотОбъект),
		            ВсегоОбработано,
		            Эластик_Сервер,
		            Эластик_Порт,
		            Эластик_Путь);
	Иначе
		Лог.Информация("[%1]: Завершение отправки данных (%2) в ""%3:%4%5""",
		               ТипЗнч(ЭтотОбъект),
		               ВсегоОбработано,
		               Эластик_Сервер,
		               Эластик_Порт,
...
Процедура ОтправитьДанные()
	...
	Лог.Отладка("[%1] Отправка пакета данных (%2) в ""%3:%4%5"".",
	            ТипЗнч(ЭтотОбъект),
...
Функция ПолучитьСоединение()
	...
	Попытка
		Эластик_Соединение = Новый HTTPСоединение(Эластик_Сервер, Эластик_Порт, Эластик_Пользователь, Эластик_Пароль, , 20);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение СтрШаблон("Ошибка установки соединения с сервером elasticsearch (%1:%2): %3",
		                            Эластик_Сервер,
		                            Эластик_Порт,
		                            ТекстОшибки);
	КонецПопытки;
...
//
Процедура УстановитьАдресСервераЭластик()
